...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // Returns the substring of str1 of 0 till the overlap and
        // concatenates str2 onto the end
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        boolean sub = false; // will indicate if precondition is met
        for (String s : strSet) {
            if (s.indexOf(str) != -1) {
                sub = true;
            }
        }
        // Only if precondition is met
        if (!sub) {
            Set<String> temp = strSet.newInstance();
            for (String s : strSet) {
                if (str.indexOf(s) >= 0) { // checks for substring overlap
                    temp.add(s);
                }
            }
            strSet.remove(temp); // remove temp from original set
            strSet.add(str); // add str to original set
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> si = new Set1L<String>(); // set to be returned
        // Until the input has not reached the end of the file, call
        // addToSetAvoidingSubstrings and input the next line
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(si, input.nextLine());
        }
        return si; // return the set
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        String temp = text; // do not want to alter the original text
        int textLen = text.length(); // used for iteration

        // Iterates and checks for tilda and creates new line in place
        for (int i = 0; i < temp.length(); i++) {
            if (temp.charAt(i) == '~') {
                String firstHalf = temp.substring(0, i); // first half of string
                String secondHalf = temp.substring(i + 1, textLen); // second half
                temp = firstHalf + "\n" + secondHalf;
            }
        }
        out.println(temp); // print statement
    }
